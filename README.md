...
> Benchmark-Suite - Open source tool by AetherCodeHQ

`benchmarking` `cli` `developer-tools` `golang` `open-source` `performance`

## What is it?

**Benchmark Suite** is a cli tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
### Install from source
```bash
git clone https://github.com/AetherCodeHQ/Benchmark-Suite.git
cd Benchmark-Suite
...
```bash
# Basic usage
./Benchmark-Suite --path ./target

# With options
./Benchmark-Suite --path ./target --format json --output report.json

# Verbose mode
./Benchmark-Suite --path ./target --verbose
```
...
### Basic scan
```bash
./Benchmark-Suite --path ./my-project
```

### JSON report
```bash
./Benchmark-Suite --path ./my-project --format json --output report.json
```
...
- name: Run Benchmark Suite
  run: ./Benchmark-Suite --path . --format json --output report.json
```
